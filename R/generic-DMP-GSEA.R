## USE gometh from {missMethyl}
##


# dmp_GSEA <- function (
#     beta = myNorm, DMP = myDMP[[1]], DMR = myDMR, CpGlist = NULL,
#     Genelist = NULL, pheno = myLoad$pd$Sample_Group, method = "fisher",
#     arraytype = "450K", Rplot = TRUE, adjPval = 0.05, cores = 1)
# {
...
#                                           "UCSC_RefGene_Name")]
#     ueid.v <- unique(unlist(sapply(RSanno$UCSC_RefGene_Name,
#                                    function(x) strsplit(x, split = ";")[[1]])))
#     bias.Data <- table(unlist(sapply(RSanno$UCSC_RefGene_Name,
#                                      function(x) unique(strsplit(x, split = ";")[[1]]))))
#     bias.Data <- as.numeric(bias.Data[ueid.v])
#     loi.lv <- list()
#     if (!is.null(DMP)) {
#       cpg.idx <- rownames(DMP)[which(DMP$adj.P.Val <= adjPval)]
#       loi.lv[["DMP"]] <- unique(unlist(sapply(RSanno[cpg.idx,
#                                                      "UCSC_RefGene_Name"], function(x) strsplit(x,
#                                                                                                 split = ";")[[1]])))
#     }
#     if (!is.null(DMR)) {
#       cpg.idx <- unique(unlist(apply(DMR[[1]], 1, function(x) rownames(RSanno)[which(RSanno$chr ==
#                                                                                        x[1] & RSanno$pos >= as.numeric(x[2]) & RSanno$pos <=
#                                                                                        as.numeric(x[3]))])))
#       loi.lv[["DMR"]] <- unique(unlist(sapply(RSanno[cpg.idx,
#                                                      "UCSC_RefGene_Name"], function(x) strsplit(x,
#                                                                                                 split = ";")[[1]])))
#     }
#     if (!is.null(CpGlist)) {
#       if (class(CpGlist) == "character") {
#         loi.lv[["CpG"]] <- unique(unlist(sapply(RSanno[CpGlist,
#                                                        "UCSC_RefGene_Name"], function(x) strsplit(x,
#                                                                                                   split = ";")[[1]])))
#       }
#       else if (class(CpGlist) == "list") {
#         if (names(CpGlist) %in% c("DMP", "DMR", "CpG",
#                                   "Gene"))
#           stop("Your CpG list can not contain names as follows: \"DMP\",\"DMR\",\"CpG\",\"Gene\".")
#         if (names(CpGlist) == NULL | sum(is.na(names(CpGlist))) !=
#             0)
#           stop("Please specify your CpG list names.")
#         cpg.tmp <- lapply(CpGlist, function(x) unique(unlist(sapply(RSanno[x,
#                                                                            "UCSC_RefGene_Name"], function(x) strsplit(x,
#                                                                                                                       split = ";")[[1]]))))
...
#         if (names(Genelist) %in% c("DMP", "DMR", "CpG",
#                                    "Gene"))
#           stop("Your Gene list can not contain names as follows: \"DMP\",\"DMR\",\"CpG\",\"Gene\".")
#         if (names(Genelist) == NULL | sum(is.na(names(Genelist))) !=
#             0)
#           stop("Please specify your Gene list names.")
...
#     nrepGEID.m[, "nRep"] <- unlist(lapply(PathwayList, function(x) length(intersect(x,
#                                                                                     ueid.v))))
#     nrepGEID.m[, "fRep"] <- nrepGEID.m[, "nRep"]/nrepGEID.m[,
#                                                             "nList"]
#     listsummary.lm <- list()
...
#         InMSigDBlist_InList <- unlist(lapply(PathwayList,
#                                              function(x) length(intersect(x, selEID.v))))
#         NotInMSigDBlist_InList <- length(selEID.v) -
#           InMSigDBlist_InList
#         InMSigDBlist_NotInList <- unlist(lapply(PathwayList,
#                                                 function(x) length(intersect(x, ueid.v)) -
#                                                   length(intersect(x, selEID.v))))
#         NotInMSigDBlist_NotInList <- length(ueid.v) -
#           InMSigDBlist_InList - NotInMSigDBlist_InList -
#           InMSigDBlist_NotInList
#         ovlapG.lv_2 <- lapply(PathwayList, function(x) intersect(x,
#                                                                  selEID.v))
#         fisher.lm_2 <- cbind(InMSigDBlist_InList, NotInMSigDBlist_InList,
#                              InMSigDBlist_NotInList, NotInMSigDBlist_NotInList)
#         listPV.v_2 <- t(apply(fisher.lm_2, 1, function(x) unlist(fisher.test(matrix(x,
#                                                                                     2, 2), alternative = "greater")[c(1, 3)])))
#         info <- cbind(fisher.lm_2, listPV.v_2)
#         selL.idx <- which(nrepGEID.m[, 3] > 0.6)
#         listsummary.m_2 <- data.frame(Gene_List = listnames.v[selL.idx],
#                                       nrepGEID.m[selL.idx, ], nOVLAP = fisher.lm_2[,
#                                                                                    1][selL.idx], OR = listPV.v_2[selL.idx, 2],
#                                       `P-value` = listPV.v_2[selL.idx, 1], adjPval = p.adjust(listPV.v_2[selL.idx,
#                                                                                                          1], method = "BH"), Genes = unlist(lapply(ovlapG.lv_2[selL.idx],
#                                                                                                                                                    function(x) PasteVector(x))))
#         listsummary.m_2 <- listsummary.m_2[order(listsummary.m_2$adjPval),
#         ]
#         pvals <- listsummary.m_2[which(listsummary.m_2$adjPval <=
#                                          adjPval), ]
#       }
...
#                      bias.data = bias.Data, plot.fit = Rplot)
#         pvals <- goseq(pwf, "hg19", "geneSymbol")
#         over_represented_adjPvalue <- p.adjust(pvals$over_represented_pvalue,
#                                                method = "BH")
...
#       message("  Calculating GSEA with gometh method on ",
#               i, " CpG list")
#       message("  Note that gometh method would count numbers of CpGs in each genes and correct this bias.")
#
#       tmp <- gometh(loi.lv[[i]], all.cpg = ueid.v, collection = c("GO"),
#                     array.type = arraytype, plot.bias = Rplot, prior.prob = TRUE)
#
...
#     if (is.null(pheno))
#       stop("Pheno parameter is required for ebayes GSEA method.")
...
#       if (length(table(pheno) == 2))
#         message("  pheno parameter is category variable, logistic method would be ued when calculating ebayes GSEA.")
...
#     }
#     listsummary.lm <- champ.ebGSEA(beta = beta, pheno = pheno,
#                                    adjPval = adjPval, minN = 5, arraytype = arraytype,
#                                    cores = cores)
#   }
#   else {
