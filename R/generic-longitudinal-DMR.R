...
#' @export
#'
computeLongitudinalDMR <- function(
    beta, covariates,
    pair_var = "patient",
    time_var = "time",
    formula=NULL,
  ...
  # rawBeta <- computeDMP(beta = beta, covariates = covariates, formula = formula, adjPVal = 1)
  rawBeta <- computeLongitudinalDMP(beta = beta, covariates = covariates, pair_var = pair_var,
                                    time_var = time_var, formula = formula, adjPVal = 1)
