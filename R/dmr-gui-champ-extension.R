#' DMR GUI for paired end analysis
#'
#' @param DMR the DMR object from champ.
#'
#' @export
#'
pairedDMR.GUI <- function (DMR = myDMR, beta = myNorm, pheno = myLoad$pd$Sample_Group, pair=NULL,
                           runDMP = TRUE, compare.group = NULL, arraytype = "450K")
{
  ...
  if (runDMP) {
    ...
    DMP <- champ.PairedDMP(beta = tmpbeta, pheno = tmppheno, pair=pair, adjPVal = 1,
  ...
  DMR[[1]]$seqnames <- as.factor(substr(DMR[[1]]$seqnames,
                                        4, 100))
  index <- apply(DMR[[1]], 1, function(x) which(probe.features$CHR ==
  ...
  innerdmrplot <- function(select, Group, dmr.idx) {
    ...
    G <- lapply(G, function(h) data.frame(Sample = rep(colnames(h),
                                                       each = nrow(h)), ID = rep(rownames(h), ncol(h)),
                                          pos = rep(as.numeric(as.factor(select$MAPINFO)),
                                                    ncol(h)), Value = as.vector(h)))
    for (i in 1:length(G)) G[[i]] <- data.frame(G[[i]], pheno = names(G)[i])
    X <- do.call(rbind, G)
    X <- cbind(X, showtext = paste("cpgID:", X$ID, " Sample:",
                                   X$Sample, sep = ""))
    p <- plot_ly()
    p <- add_trace(p, data = X, x = ~pos, y = ~Value, type = "scatter",
                   mode = "markers", text = ~showtext, color = ~pheno,
                   marker = list(opacity = 0.6, size = 4))
    message("<< Dots Plotted >>")
    Fit <- lapply(G, function(h) data.frame(pos = unique(h$pos),
                                            ID = unique(h$ID), Mean = aggregate(h$Value, by = list(h$pos),
                                                                                mean)[, 2]))
    for (i in 1:length(Fit)) Fit[[i]] <- data.frame(Fit[[i]],
                                                    pheno = paste(names(Fit)[i], "Mean"))
    df <- data.frame(x = unlist(lapply(Fit, "[[", "pos")),
                     y = unlist(lapply(Fit, "[[", "Mean")), ID = unlist(lapply(Fit,
                                                                               "[[", "ID")), cut = unlist(lapply(Fit, "[[",
                                                                                                                 "pheno")))
    p <- add_trace(p, data = df, x = ~x, y = ~y, text = ~ID,
                   type = "scatter", mode = "lines+markers", color = ~cut,
                   line = list(shape = "spline", width = 3, opacity = 0.3),
                   marker = list(size = 1))
    message("<< Mean line Plotted >>")
    Fit2 <- lapply(G, function(h) data.frame(pos = unique(h$pos),
                                             ID = unique(h$ID), Fitted = unique(fitted(loess(h$Value ~
                                                                                               h$pos)))))
    for (i in 1:length(Fit2)) Fit2[[i]] <- data.frame(Fit2[[i]],
                                                      pheno = paste(names(Fit2)[i], "Loess"))
    df2 <- data.frame(x = unlist(lapply(Fit2, "[[", "pos")),
                      y = unlist(lapply(Fit2, "[[", "Fitted")), ID = unlist(lapply(Fit2,
                                                                                   "[[", "ID")), cut = unlist(lapply(Fit2, "[[",
                                                                                                                     "pheno")))
    p <- add_trace(p, data = df2, x = ~x, y = ~y, text = ~ID,
                   type = "scatter", mode = "lines+markers", color = ~cut,
                   line = list(shape = "spline", width = 3, opacity = 0.3,
                               dash = "dash"), marker = list(size = 1))
    message("<< Loess line Plotted >>")
    regioncol <- c(`1stExon` = "#00eeee", `3'UTR` = "#8b008b",
                   `5'UTR` = "#00ee00", Body = "#ffd700", IGR = "#9e9e9e",
                   TSS1500 = "#ff3030", TSS200 = "#00688b")
    featureregion <- list()
    for (i in names(regioncol)) {
      ...
      featureregion[[i]] <- data.frame(x1 = index - 0.5,
                                       x2 = index + 0.5, y = -0.05, IDfeature = i, color = regioncol[i])
      options(warn = oldw)
    }
    cgicol <- c(island = "#ff83fa", opensea = "#ffdab9",
                shelf = "#bbffff", shore = "#98fb98")
    cgiregion <- list()
    for (i in names(cgicol)) {
      ...
      oldw <- getOption("warn")
      options(warn = -1)
      cgiregion[[i]] <- data.frame(x1 = index - 0.5, x2 = index +
                                     0.5, y = -0.1, IDfeature = i, color = cgicol[i])
      options(warn = oldw)
    }
    df <- rbind(do.call(rbind, featureregion), do.call(rbind,
                                                       cgiregion))
    colorrecord <- c()
    for (i in 1:nrow(df)) {
      legend = F
      ...
      p <- add_trace(p, x = c(df$x1[i], df$x2[i]), y = c(df$y[i],
                                                         df$y[i]), name = df$IDfeature[i], mode = "lines",
                     line = list(color = df$color[i], width = 10,
                                 opacity = 0.5), showlegend = legend, hoverinfo = "text",
                     text = df$IDfeature[i], type = "scatter")
    }
  ...
  innergeneenrichplot <- function(select) {
    ...
    if (c("adj.P.Val") %in% colnames(select)) {
      ...
      h <- table(as.character(select$gene), paste(select$t,
                                                  select$adj.P.Val))
      h <- h[order(rowSums(h), decreasing = T), ]
      h <- cbind(h[, c(4, 2)], rowSums(h[, c(1, 3)]))
      colnames(h) <- c("Hyper", "Hypo", "Not Significance")
      h <- data.frame(Variance = rep(colnames(h), each = nrow(h)),
                      gene = rep(rownames(h), ncol(h)), Number = as.vector(h))
    ...
    p <- layout(p, title = paste("All ", length(unique(h$gene)),
                                 " Gene Enriched by DMR-related CpGs", sep = ""),
                margin = m, barmode = "stack")
  }
  innerheatmap <- function(data) {
    m = list(l = 170, r = 70, b = 60, t = 50, pad = 10)
    p <- plot_ly(z = data, x = colnames(data), y = rownames(data),
                 type = "heatmap")
    p <- layout(p, title = paste("Heatmap for ", nrow(data),
                                 " CpGs in all DMRs", sep = ""), margin = m)
  }
  innertestplot <- function() {
    plot_ly(x = 1:10, y = 1:10)
  }
  app <- shinyApp(ui = fluidPage(tags$head(tags$style("#dmrplot{height:40vh !important;}")),
                                 ...
                                 theme = shinytheme("readable"), titlePanel(paste(names(DMR)[1],
                                                                                  "Overview")), sidebarLayout(sidebarPanel(br(), width = 3,
                                                                                                                           sliderInput("pvaluebin", "P value Cutoff:", min = 0,
                                                                                                                                       max = 1, step = 0.025, value = 0.05), sliderInput("minprobebin",
                                                                                                                                                                                         "Min Number Probes:", min = 1, max = 100, step = 1,
                                                                                                                                                                                         value = 7), actionButton("go", "Submit"), br(),
                                                                                                                           br(), br(), sliderInput("dmrbin", "DMR Index:", min = 1,
                                                                                                                                                   max = nrow(sig), step = 1, value = 1), actionButton("dmrsearch",
                                                                                                                                                                                                       "Submit")), mainPanel(tabsetPanel(tabPanel("DMRtable",
                                                                                                                                                                                                                                                  align = "left", div(DT::dataTableOutput("table"),
                                                                                                                                                                                                                                                                      style = "font-size:75%")), tabPanel("CpGtable",
                                                                                                                                                                                                                                                                                                          align = "left", div(DT::dataTableOutput("cpgtable"),
                                                                                                                                                                                                                                                                                                                              style = "font-size:75%")), tabPanel("DMRPlot",
                                                                                                                                                                                                                                                                                                                                                                  align = "center", fluidRow(align = "center", br(),
                                                                                                                                                                                                                                                                                                                                                                                             column(width = 12, align = "left", div(DT::dataTableOutput("probetable"),
                                                                                                                                                                                                                                                                                                                                                                                                                                    style = "font-size:75%")), column(width = 12,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                      align = "center", plotlyOutput("dmrplot")))),
                                                                                                                                                                                                                                         tabPanel("Summary", align = "center", fluidRow(align = "center",
                                                                                                                                                                                                                                                                                        br(), column(width = 12, align = "center", plotlyOutput("geneenrichplot")),
                                                                                                                                                                                                                                                                                        column(width = 12, align = "center", plotlyOutput("heatmap")))))))),
                  server = function(input, output) {
                    DMR_repalce <- eventReactive(input$dmrsearch, {
                      ...
                      mydmrselect <- Anno[Anno$DMRindex == paste("DMR",
                                                                 dmr.idx, sep = "_"), ]
                      mydmrselect <- mydmrselect[order(mydmrselect$MAPINFO),
                      ]
                      mygroup <- split(as.data.frame(t(beta[rownames(mydmrselect),
                      ])), pheno)
                      list(mydmrselect = mydmrselect, mygroup = mygroup,
                           dmr.idx = dmr.idx)
                    })
                    Cutoff_repalce <- eventReactive(input$go, {
                      ...
                      pvalueCutoff <- as.numeric(input$pvaluebin)
                      minprobeCutoff <- as.numeric(input$minprobebin)
                      mydmr <- DMR[[1]][which(sig$DMR.pvalue <= pvalueCutoff &
                                                sig$DMR.probes >= minprobeCutoff), ]
                      mygeneselect <- Anno[which(Anno$DMRindex %in%
                                                   rownames(mydmr)), ]
                      if (runDMP)
                        mygeneselect <- data.frame(mygeneselect, DMP[rownames(mygeneselect),
                                                                     c("t", "adj.P.Val")])
                      mydata <- beta[rownames(mygeneselect), ]
                      rownames(mydata) <- paste(mygeneselect$DMRindex,
                                                rownames(mydata), sep = "-")
                      list(pvalueCutoff = pvalueCutoff, minprobeCutoff = minprobeCutoff,
                           mydmr = mydmr, mygeneselect = mygeneselect,
                           mydata = mydata)
                    })
                    output$probetable <- DT::renderDataTable({
                      datatable <- DMR_repalce()$mydmrselect
                      datatable <- data.frame(ID = rownames(datatable),
                                              datatable)
                    ...
                    output$dmrplot <- renderPlotly({
                      innerdmrplot(DMR_repalce()$mydmrselect, DMR_repalce()$mygroup,
                                   DMR_repalce()$dmr.idx)
                    })
